...
<?php
			...
			$query3 = mysqli_query($database->dblink,"SELECT * FROM ".TB_PREFIX."users WHERE id = ".$session->uid);
	...
	if($data3['dailygold'] == 1 && $session->access > 1)
	{
		...
		{
			$entsperren = mysqli_query($database->dblink,"UPDATE ".TB_PREFIX."users SET dailygold = '0' WHERE id = ".$session->uid);
			$entsperren2 = mysqli_query($database->dblink,"UPDATE ".TB_PREFIX."users SET dailygoldtime = '0' WHERE id = ".$session->uid) or die(mysql_error());
...

	{
		...
		if($session->access != 0)
		{
			$goldok = mysqli_query($database->dblink,"UPDATE ".TB_PREFIX."users SET gold=gold+50 WHERE id = ".$session->uid);
			echo "UAUUUU 50 Gold. In about 24 hours you can go back for more ...";
		}

		$dailytime=time(60*60*24);
		$gold1= mysqli_query($database->dblink,"UPDATE ".TB_PREFIX."users SET dailygold = '1' WHERE id= ".$session->uid);
		$goldtime= mysqli_query($database->dblink,"UPDATE ".TB_PREFIX."users SET dailygoldtime='".$dailytime."' WHERE id = ".$session->uid) or die(mysql_error());
	}
