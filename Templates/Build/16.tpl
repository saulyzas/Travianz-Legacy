...
<tbody class=\"infos\"><tr><th>".UPKEEP."</th><td colspan=\"$colspan\"><div class='sup'>".$technology->getUpkeep($enforce, $tribe)."<img class=\"r4\" src=\"img/x.gif\" title=\"Crop\" alt=\"Crop\" />".PER_HR."</div><div class='sback'><a href='a2b.php?r=".$enforce['id']."'>".SEND_BACK."</a></div></td></tr>";
    ...
    // *** PRASIDEDA PAKEISTAS KODAS: getPrisoners3 ***
    if(count($database->getPrisoners3($village->wid)) > 0){
        echo "<h4>".PRISONERS."</h4>";
        foreach($database->getPrisoners3($village->wid) as $prisoners){
            // PATIKRINAME, AR KAIMO ID YRA TEISINGAS
            if ($prisoners['from'] > 0) {
                $colspan = 10 + $prisoners['t11'];
                $colspan2 = $colspan + 1;

                $toVillageId = $prisoners['wref']; // Kaimas, kuriame kaliniams yra
                $toVillageName = $database->getVillageField($toVillageId, "name");
                if (empty($toVillageName)) {
                    $toVillageName = "Nežinomas kaimas (ID: ".$toVillageId.")";
                }

                // SAUGIAI GAUNAME GENTIES ID
                $ownerId = $database->getVillageField($prisoners['from'], "owner");
                $tribe = 0; 
                if ($ownerId > 0) {
                    $tribe = $database->getUserField($ownerId, "tribe", 0);
                }

                // JEI GENTIS NUSTATYTA TEISINGAI, VAIZDUOJAME LENTELĘ
                if ($tribe > 0) {
                    echo "<table class=\"troop_details\" cellpadding=\"1\" cellspacing=\"1\"><thead><tr><td class=\"role\">
    <a href=\"karte.php?d=".$toVillageId."&c=".$generator->getMapCheck($toVillageId)."\">".$toVillageName."</a></td>
    <td colspan=\"$colspan\">";
                    echo "<a href=\"karte.php?d=".$toVillageId."&c=".$generator->getMapCheck($toVillageId)."\">".PRISONERSIN." ".$toVillageName."</a>";
                    echo "</td></tr></thead><tbody class=\"units\">";

                    echo "<tr><th>&nbsp;</th>";
                    for($i = 0; $i < 10; $i++){
                        $unitID = ($tribe - 1) * 10 + $i + 1;
                        echo "<td><img src=\"img/x.gif\" class=\"unit u$unitID\" title=\"".$technology->getUnitName($unitID)."\" alt=\"".$technology->getUnitName($unitID)."\" /></td>";
                    }
                    if($prisoners['t11'] != 0){
                        echo "<td><img src=\"img/x.gif\" class=\"unit uhero\" title=\"Hero\" alt=\"Hero\" /></td>";
                    }
                    ...
                    for($i = 0; $i < 10; $i++){
                        $dataKey = 't' . ($i + 1);
                        if($prisoners[$dataKey] == 0){
                            echo "<td class=\"none\">";
                        } else {
                            echo "<td>";
                        }
                        echo $prisoners[$dataKey]."</td>";
                    }
        ...
        <tbody class=\"infos\"><tr><th>".UPKEEP."</th><td colspan=\"$colspan2\"><div class='sup'>".$technology->getUpkeep($prisoners, $tribe, 0, 1)."<img class=\"r4\" src=\"img/x.gif\" title=\"Crop\" alt=\"Crop\" />".PER_HR."</div><div class='sback'><a href='a2b.php?delprisoners=".$prisoners['id']."'>".KILL."</a></div></td></tr>";
                    ...
                    echo "<p style='color:red; padding-left: 10px;'>Kalinių iš nežinomos genties. Negalima atvaizduoti karių.</p>";
                }
            }
            // *** PAKEITIMAS: ELSE DALIS, KURI RODĖ KLaidą, BUVO PAŠALINTA, KAD PRANEŠIMAS NERODYTŲSI ***
        }
    }
    // *** BAIGIASI PAKEISTAS KODAS: getPrisoners3 ***

    // *** PRASIDEDA PAKEISTAS KODAS: getPrisoners ***
    if(count($database->getPrisoners($village->wid)) > 0){
        ...
        foreach($database->getPrisoners($village->wid) as $prisoners){
            ...
            if ($prisoners['from'] > 0) {
                ...
                $fromVillageId = $prisoners['from'];
                $fromVillageName = $database->getVillageField($fromVillageId, "name");
                if (empty($fromVillageName)) {
                    $fromVillageName = "Nežinomas kaimas (ID: ".$fromVillageId.")";
                }
                
                // SAUGIAI GAUNAME GENTIES ID
                $ownerId = $database->getVillageField($fromVillageId, "owner");
                $tribe = 0; 
                if ($ownerId > 0) {
                    $tribe = $database->getUserField($ownerId, "tribe", 0);
                }
                ...
                if ($tribe > 0) {
                    echo "<table class=\"troop_details\" cellpadding=\"1\" cellspacing=\"1\"><thead><tr><td class=\"role\">
    <a href=\"karte.php?d=".$fromVillageId."&c=".$generator->getMapCheck($fromVillageId)."\">".$fromVillageName."</a></td>
    <td colspan=\"$colspan\">";
                    echo "<a href=\"karte.php?d=".$fromVillageId."&c=".$generator->getMapCheck($fromVillageId)."\">".PRISONERSFROM." ".$fromVillageName."</a>";
                    echo "</td></tr></thead><tbody class=\"units\">";
        ...
        <tbody class=\"infos\"><tr><td colspan=\"$colspan2\"><div class='sup'><img class=\"r6\" src=\"img/x.gif\" title=\"Crop\" alt=\"Crop\" /></div><div class='sback'><a href='a2b.php?delprisoners=".$prisoners['id']."'>".SEND_BACK."</a></div></td></tr>";
                    ...
                    echo "<p style='color:red; padding-left: 10px;'>Kalinių iš nežinomos genties (iš kaimo ".$fromVillageName."). Negalima atvaizduoti karių.</p>";
                }
            }
            // *** PAKEITIMAS: ELSE DALIS, KURI RODĖ KLaidą, BUVO PAŠALINTA, KAD PRANEŠIMAS NERODYTŲSI ***
        }
    }
    // *** BAIGIASI PAKEISTAS KODAS: getPrisoners ***
    ?>
